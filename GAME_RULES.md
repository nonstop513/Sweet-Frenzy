...
**掉落规则**：符号向**左侧**掉落，新符号从**右侧**填充

**示例（Row 0: 列坐标 [3, 5, 7, 9]）**：
```
消除前：  [M1] [--] [M2] [M3]
         (3)  (5)  (7)  (9)
...
**Row 3 (中央Wild行) 示例**：
```
列坐标：  [0]  [2]  [4] [6W] [8]  [10] [12]

消除前：  [M1] [M2] [--] [W]  [--] [M3] [M4]
掉落后：  [M1] [M2] [--] [W]  [M3] [M4] [--]  
         (Wild固定不动，左右两侧独立掉落)
填充后：  [M1] [M2] [M5] [W]  [M3] [M4] [M6]
```
...
4. 限制：
   - 不能覆盖中央Wild [3,6]
   - 不能互相覆盖
   - **C1 处理**：如果 4 格中有 C1，保持 C1 不变，只修改其他格子
   - 第一个钻石必须与现有相同符号相邻

**C1 处理示例**：
```
假设选中的钻石 4 格为：[2,5][2,3][3,4][3,6]
其中 [2,5] 是 C1

放置结果：
- [2,5]: 保持 C1 不变
- [2,3]: 改为 Mega 符号 (例如 M3)
- [3,4]: 改为 Mega 符号 (M3)
- [3,6]: 改为 Mega 符号 (M3)

最终形成 3+1 的混合钻石（3格Mega + 1格C1）
```
...
```javascript
const c1Count = countC1OnBoard();  // 统计盘面上所有 C1 (board[r,c] == 1)
const retriggerMap = {
    3: 10,   // 3个C1 → +10 spins
    4: 12,   // 4个C1 → +12 spins
    5: 15,   // 5个C1 → +15 spins
...
dropSymbols();  // 符号向左掉落：[右→左]
fillEmpty(eliminate_count=1);  // 使用DropWeight第0行，从右侧填充
