    ...
    def try_eliminate_feature(self):
        ...
        for _ in range(num_blocks):
            ...
            for center_row, center_col in all_possible_centers:
                ...
                # 限制3: 不能覆盖C1符号（编号为1）
                # 注意：这会排除所有会覆盖到C1的中心点
                # 例如：若[1,8]是C1，则[1,8][1,10][0,7][0,9]都不能作为中心点
...
def basegame(rounds):
    ...
    rounds: 游戏轮数
    返回: (每轮的分数列表, 每轮结束时的C1数量列表, 每轮初始版面的C1数量列表)
    """
    scores = np.zeros(rounds, dtype=np.int64)
    c1_counts = np.zeros(rounds, dtype=np.int32)
    initial_c1_counts = np.zeros(rounds, dtype=np.int32)  # 新增：初始版面C1数量
